...
def build_cases(base_cfg: dict) -> list[tuple[str, dict]]:
    ...
    add("arch_gqa_4", [(("model", "attn_kv_heads"), 4)])
    add("arch_gqa_2", [(("model", "attn_kv_heads"), 2)])
    add("arch_gqa_1", [(("model", "attn_kv_heads"), 1)])
    return cases
...
def main() -> None:
    ...
    set_nested(base_cfg, ("quantize", "pack_order"), "size_desc")
    ...
    out = root / "runs" / "arch_sweep_summary.json"
