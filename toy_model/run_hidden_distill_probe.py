...
def main() -> None:
    ...
    v0["distill"]["hidden_enabled"] = False
    variants.append(("hidden_probe_base", v0))

    v1 = copy.deepcopy(base)
    v1["distill"]["hidden_enabled"] = True
    v1["distill"]["hidden_weight"] = 0.05
    variants.append(("hidden_probe_w005", v1))

    v2 = copy.deepcopy(base)
    v2["distill"]["hidden_enabled"] = True
    v2["distill"]["hidden_weight"] = 0.10
    variants.append(("hidden_probe_w010", v2))
    ...
    base_m = next(r for r in results if r["run_name"] == "hidden_probe_base")
    deltas = []
    for r in results:
        if r["run_name"] == "hidden_probe_base":
            continue
        deltas.append(
            {
                "run_name": r["run_name"],
                ...
                "hidden_weight": r.get("distill_hidden_weight"),
            }
        )

    summary = {"results": results, "deltas_vs_base": deltas}
    out = root / "runs" / "hidden_distill_probe_summary.json"
