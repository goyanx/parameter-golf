...
def build_cases(base_cfg: dict) -> list[tuple[str, str, dict]]:
    ...
    add(
        ...
        [
            (("lowrank", "rank"), 48),
            (("lowrank", "include_patterns"), ["attn", "mlp"]),
            (("lowrank", "exclude_patterns"), ["head", "token_emb"]),
        ],
    )

    # Artifact packing order sweep.
    add("packing", "pack_state_dict", [])
    add("packing", "pack_name", [(("quantize", "pack_order"), "name")])
    add("packing", "pack_size_desc", [(("quantize", "pack_order"), "size_desc")])
...
def main() -> None:
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as ex:
        futs = [ex.submit(run_one, root, python_exe, name, cfg, group) for group, name, cfg in cases]
    ...
    out = root / "runs" / "breadth_sweep_summary.json"
