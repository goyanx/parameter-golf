...
def build_cases(base_cfg: dict) -> list[tuple[str, dict]]:
    ...
    for idx, prune_amt in enumerate([0.06, 0.08, 0.10, 0.12, 0.14], start=1):
        cfg = copy.deepcopy(base_cfg)
        cfg["prune"]["amount"] = prune_amt
        cfg["prune"]["mode"] = "row"
        cfg["prune"]["include_patterns"] = ["attn"]
        cfg["prune"]["progressive"] = True
        cfg["prune"]["start_step"] = 100
        cfg["prune"]["end_step"] = 200
        cfg["prune"]["every"] = 25
        cfg["quantize"]["group_size"] = 40
        cfg["lowrank"]["rank"] = 56
        cfg["qat"]["steps"] = 90
        cfg["qat"]["delay_fake_quant_steps"] = 20
        cfg["distill"]["teacher_steps"] = 240
        cases.append((f"struct_tight_{idx:02d}", cfg))
    return cases
...
def main() -> None:
    ...
    base_cfg = yaml.safe_load((root / "config_structured_best.yaml").read_text(encoding="utf-8"))
    cases = build_cases(base_cfg)

    max_workers = max(1, int(os.environ.get("MAX_WORKERS", "2")))
    results: list[dict] = []
    print(f"Running {len(cases)} structured cases with max_workers={max_workers}")
    ...
    ok_sorted = sorted(ok, key=lambda r: (r["val_loss"], r["artifact_size_mb"]))
    size_sorted = sorted(ok, key=lambda r: (r["artifact_size_mb"], r["val_loss"]))
    ...
    summary = {
        ...
        "best_size": size_sorted[0] if size_sorted else None,
        "all_results": sorted(ok, key=lambda r: r["run_name"]) + failed,
    }
    out = root / "runs" / "structured_tight_summary.json"
