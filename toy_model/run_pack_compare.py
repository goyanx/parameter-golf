...
def build_cases(base_cfg: dict) -> list[tuple[str, dict]]:
    ...
    add("pack_state_dict_struct", [])
    add("pack_name_struct", [(("quantize", "pack_order"), "name")])
    add("pack_size_desc_struct", [(("quantize", "pack_order"), "size_desc")])
    return cases
...
def main() -> None:
    ...
    out = root / "runs" / "pack_compare_summary.json"
